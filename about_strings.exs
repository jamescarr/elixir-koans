...
defmodule About_Strings do
  ...
  think "what does the string start with?" do
    a_string = "Hello there"
    assert String.starts_with?(a_string, "H")
    assert String.starts_with?(a_string, ["Bonjour", "Hello"])
    refute String.starts_with?(a_string, ["Bonjour", "Greetings"])
  end

  think "what does the string end with?" do
    a_string = "Live long and prosper"
    assert String.ends_with?(a_string, "prosper")
    assert String.ends_with?(a_string, ["multiply", "prosper"])
    refute String.ends_with?(a_string, ["keep calm"])
  end

  think "does a string contain something?" do
    a_string = "May the force be with you"
    assert String.contains?(a_string, "force")
    assert String.contains?(a_string, ["voyager", "you"])
    refute String.contains?(a_string, ["prosper"])
  end
