...
defmodule About_Enums do
  ...
  think "Check if any items match" do
    list = [1, 2, 3]
    assert Enum.any?(list, fn (x) -> x < 2 end)
  end

  think "Is it there, or not?" do
    list = [:a, :b, :c]
    refute Enum.member?(list, :d)
  end
  ...
  think "finding and manipulating a value" do
    list = [1, 2, 3]
    assert Enum.find_value(list, fn (x) -> rem(x, 2) == 1 end)
  end
  ...
  think "map reduce" do
    list = [2, 3, 4]
    assert Enum.map_reduce(list, 0, fn (x, acc) -> {x * 2, x + acc} end) == {[4, 6, 8], 9}
  end
  ...
  think "taking some items the other way" do
    numbers = 1..5
    assert Enum.take(numbers, -2) == [4, 5]
  end
